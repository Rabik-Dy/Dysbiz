...
class kitchen_screen_dashboard extends Component {
  setup() {
    ...
    onWillStart(() => {
      this.busService.addEventListener(
        "notification",
        this.onPosOrderCreation.bind(this)
      );
    });
    ...
    self.orm
      .call("pos.order", "get_details", ["", self.shop_id, ""])
      .then(function (result) {
        self.state.order_details.splice(
          0,
          self.state.order_details.length,
          ...filteredOrders
        );
        ...
        self.state.draft_count = self.state.order_details.filter(
          (order) =>
            order.order_status == "draft" &&
            order.config_id[0] == self.state.shop_id
        ).length;
        self.state.waiting_count = self.state.order_details.filter(
          (order) =>
            order.order_status == "waiting" &&
            order.config_id[0] == self.state.shop_id
        ).length;
        self.state.ready_count = self.state.order_details.filter(
          (order) =>
  ...
  onPosOrderCreation(message) {
    ...
    if (
      payload.message == "pos_order_created" &&
      payload.res_model == "pos.order"
    ) {
      this.orm.call
        .call("pos.order", "get_details", ["", self.shop_id, ""])
        .then(function (result) {
          self.state.order_details = result["orders"].filter(
            (order) =>
              order.session_id &&
              self.state.session_ids.includes(order.session_id[0])
          );
          self.state.lines = result["order_lines"];
          self.state.shop_id = self.shop_id;
          self.state.draft_count = self.state.order_details.filter(
        ...
        .catch((err) => {
          console.error("Failed to fetch POS order details:", err);
        });
    }
  }
  ...
  cancel_order(e) {
    ...
    const input_id = e.target.dataset.id;
    this.orm.call("pos.order", "order_progress_cancel", [Number(input_id)]);
    var current_order = this.state.order_details.filter(
      (order) => order.id == input_id
    );
    if (current_order) {
  ...
  accept_order_line(e) {
    var input_id = $("#" + e.target.id).val();
    this.orm.call("pos.order.line", "order_progress_change", [
      Number(input_id),
    ]);
    var current_order_line = this.state.lines.filter(
      (order_line) => order_line.id == input_id
    );
...
registry
  .category("actions")
  .add("kitchen_custom_dashboard_tags", kitchen_screen_dashboard);
